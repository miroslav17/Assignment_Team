...
import com.example.assignment_team.core.plant.TeamFactory;
...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public List<Team> findAllTeamsWithOverlap() {
        ...
        for (int i = Constants.INDEX_ZERO; i < allRecords.size() - Constants.ONE; i++) {
            for (int j = i + Constants.ONE; j < allRecords.size(); j++) {
                ...
                if (firstEmpl.getProjectId() == secondEmpl.getProjectId()
                    ...
                    if (overlapDays > Constants.DEFAULT_OVERLAP_ZERO_DAYS) {
